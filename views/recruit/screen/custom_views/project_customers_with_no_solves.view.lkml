    ...
    derived_table: {
      sql: WITH ever_paid_companies_inc_tcs AS (---- Ever Paid Customers | TCS LND/ Internal accounts Excluded
       ...
       hr_analytics.global.dim_recruit_company rc  inner join ever_paid ep on ep.company_id=rc.company_id
        ...
        WHERE ((ever_paid_companies_inc_tcs.company_stripe_plan ) <> 'free' AND (ever_paid_companies_inc_tcs.company_stripe_plan ) <> 'trial'
        AND (ever_paid_companies_inc_tcs.company_stripe_plan ) <> 'locked' OR (ever_paid_companies_inc_tcs.company_stripe_plan ) IS NULL)
