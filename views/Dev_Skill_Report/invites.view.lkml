  ...
  derived_table: {
    sql: WITH ever_paid_companies_inc_tcs AS (---- Ever Paid Customers | TCS LND/ Internal accounts Excluded
          ...
          (
            ...
            select distinct company_id from recruit_rs_replica.recruit.recruit_company_plan_changelogs
            ...
            and type not in ('free', 'trial','locked')  -- # using this logic to cover paid customers who are not covered in the above logic [company_plan_changelog table]
           ...
           recruit_rs_replica.recruit.recruit_companies rc  inner join ever_paid ep on ep.company_id=rc.id
            ...
            where lower(name) not in ('none', ' ', 'hackerrank','interviewstreet') --- Filter internal accounts based on company names
            and lower(name) not like '%hackerrank%'
            and lower(name) not like '%hacker%rank%'
            and lower(name) not like '%interviewstreet%'
            and lower(name) not like '%interview%street%'
            and name not like 'Company%'

            and rc.id not in (106529,46242) --- exclude internal test setters
            )
      SELECT
